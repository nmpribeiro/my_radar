...
export const RawFilter: React.FC = () => {
  const {
    state: {
      [RadarStateLabel.STATE]: { blips, disasterTypeFilter, useCaseFilter },
    },
    dispatch,
  ...
  const onFilterHnalder = () => {
    // selected?
    dispatch(actions.setUseCaseFilter(selectedUserCase));
    dispatch(actions.setDisasterTypeFilter(selectedDisasterType));
  };
