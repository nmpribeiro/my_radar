...
export const Blips: React.FC<Props> = ({ scaleFactor = 1, blipSize = 1, quadrant = null }) => {
  const {
    state: {
  ...
  const setHoveredItem = (item: BlipWithQuadrantKey | null) => dispatch(actions.setHoveredItem(item));
  const setSelectedItem = (item: BlipWithQuadrantKey | null) => dispatch(actions.setSelectedItem(item));
